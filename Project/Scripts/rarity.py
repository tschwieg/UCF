...
def PrintGunToFile(line, newfile, prob, rarity, vanilla):
    splitter = line.split("|")
    ...
    if(vanilla):
        #print("At the check")
        if(len(conditions) > 0 and "vanilla.csv" in conditions[0]):
            #print("Writing vanilla")
            newfile.write(gun + "," + skin + "," + "vanilla," +
                          str(prob) + "," + str(rarity) + "\n")
        return
    ...
    if(facNew):
        if("FactoryNew" in stConditions):
            newfile.write(gun + "," + skin + "," + "FactoryNew," +
                          str(.07 * prob * .9 / totalFloat) + "," + str(rarity) + "\n")
            newfile.write("StatTrak/" + gun + "," + skin + "," +
                          "FactoryNew," + str(.07 * prob * .1 / totalFloat) + "," + str(rarity) + "\n")
        else:
            newfile.write(gun + "," + skin + "," + "FactoryNew," +
                          str(.07 * prob / totalFloat) + "," + str(rarity) + "\n")

    if(minWear):
        if("MinimalWear" in stConditions):
            newfile.write(gun + "," + skin + "," + "MinimalWear," +
                          str(.08 * prob * .9 / totalFloat) + "," + str(rarity) + "\n")
            newfile.write("StatTrak/" + gun + "," + skin + "," +
                          "MinimalWear," + str(.08 * prob * .1 / totalFloat) + "," + str(rarity) + "\n")
        else:
            newfile.write(gun + "," + skin + "," + "MinimalWear," +
                          str(.08 * prob / totalFloat) + "," + str(rarity) + "\n")
    if(fieldTest):
        if("Field-Tested" in stConditions):
            newfile.write(gun + "," + skin + "," + "Field-Tested," +
                          str(.23 * prob * .9 / totalFloat) + "," + str(rarity) + "\n")
            newfile.write("StatTrak/" + gun + "," + skin + "," +
                          "Field-Tested," + str(.23 * prob * .1 / totalFloat) + "," + str(rarity) + "\n")
        else:
            newfile.write(gun + "," + skin + "," + "Field-Tested," +
                          str(.23 * prob / totalFloat) + "," + str(rarity) + "\n")
    if(wellWorn):
        if("Well-Worn" in stConditions):
            newfile.write(gun + "," + skin + "," + "Well-Worn," +
                          str(.07 * prob * .9 / totalFloat) + "," + str(rarity) + "\n")
            newfile.write("StatTrak/" + gun + "," + skin + "," +
                          "Well-Worn," + str(.07 * prob * .1 / totalFloat) + "," + str(rarity) + "\n")
        else:
            newfile.write(gun + "," + skin + "," + "Well-Worn," +
                          str(.07 * prob / totalFloat) + "," + str(rarity) + "\n")
    if(batScar):
        if("Battle-Scarred" in stConditions):
            newfile.write(gun + "," + skin + "," + "Battle-Scarred," +
                          str(.55 * prob * .9 / totalFloat) + "," + str(rarity) + "\n")
            newfile.write("StatTrak/" + gun + "," + skin + "," +
                          "Battle-Scarred," + str(.55 * prob * .1 / totalFloat) + "," + str(rarity) + "\n")
        else:
            newfile.write(gun + "," + skin + "," + "Battle-Scarred," +
                          str(.55 * prob / totalFloat) + "," + str(rarity) + "\n")


#files = glob.glob("/root/Classes/Behavioral/Paper/knives/*.txt")
files = glob.glob("../knives/*.txt")

#csvfile = open("/root/Classes/Behavioral/Paper/knives/knives.csv")
csvfile = open("../knives/knives.csv")
...
for filename in files:
    with open(filename) as f:
        ...
        for knive in knives:
            ...
            if("vanilla" in knive):
                PrintGunToFile(knive, newfile, prob, 1, True)
            else:
                PrintGunToFile(knive, newfile, prob, 1, False)
        ...
        for i in range(1, numItems):
            ...
            if(i <= red):
                prob = .0064 / float(red)
                PrintGunToFile(lines[-i], newfile, prob, 2, False)
            elif(i <= red + pink):
                prob = .032 / float(pink)
                PrintGunToFile(lines[-i], newfile, prob, 3, False)
            elif(i <= red + pink + purple):
                prob = .1598 / float(purple)
                PrintGunToFile(lines[-i], newfile, prob, 4, False)
            else:
                prob = .7992 / float(blue)
                PrintGunToFile(lines[-i], newfile, prob, 5, False)
