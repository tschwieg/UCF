...
function NewtonConsumeFunc( x::Real, cLow::Float64, cHigh::Float64, curK::Float64, gamma::Float64, beta::Float64, alpha::Float64, A::Vector{Float64}, l::Vector{Float64}, w::Vector{Float64}, k::Int64, a::Float64, b::Float64, μ::Float64, σ::Float64, degree::Int64 )
...
function OptimizeConsumption( cLow::Float64, cHigh::Float64, cGuess::Float64, curK::Float64, gamma::Float64, beta::Float64, alpha::Float64, A::Vector{Float64}, l::Vector{Float64}, w::Vector{Float64}, k::Int64, a::Float64, b::Float64, μ::Float64, σ::Float64, degree::Int64 )
    #Since we are not allowed to pass any parameters to f, we use the closure to get around this nicely.
    f(x) = NewtonConsumeFunc(x, cLow, cHigh, curK, gamma, beta, alpha, A, l, w, k,a,b, μ, σ, degree)
...
function RunDynProg()
    ...
    #cStar = kStar^alpha-delta*kStar
    v = zeros(k)#ones(k)*cStar*(1/(1-beta))
    for i in 1:k
    ...
    while( norm(prevV - v) > Epsilon )
        ...
        for i in 1:k
            ...
            OptBoy = OptimizeConsumption( cLow, cHigh, curK^alpha-delta*curK, curK, gamma, beta, alpha, A, l, w, k, a, b, μ, σ, degree )
            v[i] = OptBoy[1]
            cVec[i] = OptBoy[2]
        end
