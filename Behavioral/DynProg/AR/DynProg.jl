...
function NewtonConsumeFunc( x::Real, cLow::Float64, cHigh::Float64, curK::Float64, gamma::Float64,
                            ...
                            w::Vector{Float64}, k::Int64, a::Float64, b::Float64, μ::Float64, σ::Float64, degree::Int64 )
...
function OptimizeConsumption( cLow::Float64, cHigh::Float64, cGuess::Float64, curK::Float64,
                              ...
                              b::Float64, μ::Float64, σ::Float64, degree::Int64 )
...
function RunDynProg()
    ...
    #s is the quantiles of a lognormal with average one, and std dev 1/4
    s = zeros(100)
    #c stores the consumption paths for all the different values of s
    c = zeros(99,1000)
    for j in 1:99
        s[j] = quantile.( LogNormal( μ, σ), j/100)
        ...
        while( norm(prevV - v) > Epsilon )
            ...
            for i in 1:k
                ...
                curK = cap[i]
                cLow = 0.
                cHigh = s[j]*curK^alpha + (1-delta)*curK
                ...
                OptBoy = OptimizeConsumption( cLow, cHigh, (cLow+cHigh)/2.0, curK, gamma, beta,
                                              alpha, A, l, w, k, a, b, ρ*log(s[j]), σ, degree )
                v[i] = OptBoy[1]
                cVec[i] = OptBoy[2]
            end
            if( any(isnan, v) )
        ...
        for i in 1:1000
            c[j,i] = EvalChebyCoeff(MoveToChebyDomain(x[i],a,b),C, k-1)
        end
    end
